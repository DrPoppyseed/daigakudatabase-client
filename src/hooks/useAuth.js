...
export const likeSchoolById = async ipeds_unitid => {
    ...
    const token = await user.getIdToken(true)
    const { data } = await axios.get(
        `${import.meta.env.VITE_BACKEND_API_ENDPOINT}/users/like?ipeds_unitid=${ipeds_unitid}`, {
...
export const unlikeSchoolById = async ipeds_unitid => {
    ...
    const { data } = await axios.get(
        `${import.meta.env.VITE_BACKEND_API_ENDPOINT}/users/unlike?ipeds_unitid=${ipeds_unitid}`, {
            headers: { Authorization: `Bearer ${token}` },
        }
    )
    return data
